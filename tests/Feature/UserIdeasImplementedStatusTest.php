...
class UserIdeasImplementedStatusTest extends TestCase
{
    /**
    ...
    public function My_ideas_displaying_correctly()
    {
        ...
        $this->actingAs($user)
            ->get("/statusfilter/implemented?user=true")
    ...
    public function my_ideas_paginating_correctly()
    {
        ...
        Idea::factory(10)->create(["user_id" => $user->id, "status_id" => $statusImplemented->id]);
        ...
        $this->actingAs($user)
            ->get("/statusfilter/implemented?user=true&page=2")
    ...
    public function ideas_filter_on_all_status_based_on_categories_for_user_displaying()
    {
        ...
        $this->actingAs($user)
            ->get("/statusfilter/implemented?user=true&category=1")
    ...
    public function ideas_filter_on_all_status_based_on_categories_for_user_pagination_working()
    {
        ...
        $this->actingAs($user)
            ->get("/statusfilter/implemented?user=true&category=1&page=2")
            ->assertInertia(function (Assert $page) use ($idea) {
                $page->component("HomePage")
                    ->has("ideas", function (Assert $page) use ($idea) {
                        // dd($page);
    ...
    public function Guest_when_goes_to_user_filtered_endpoint_they_see_a_login_page()
    {
        ...
        $user = User::factory()->create();
        $otherUser = User::factory()->create();
        Idea::factory(35)->create([
            "user_id" => $otherUser->id,
            "category_id" => $cat1->id,
            "status_id" => $statusOpen->id
        ]);

        $response = $this->get("/statusfilter/implemented?user=true&category=1&page=2");
